...
OUTPUT=/tmp/arglist_test_output_$$_

########## Run test ###########################################################

run_arglist_file() {
  [ "$IFS" ] && IFS_old=$IFS
  IFS='|'; read OPTIONS INPUT EXPECTED_EXIT < $TEST
  [ "$IFS_old" ] && IFS=$IFS_old

  INPUT=`echo $INPUT`                   # trims whitespace
  EXPECTED_EXIT=`echo $EXPECTED_EXIT`   # trims whitespace
  EXPECTED_OUTPUT="$EXPECTED_DIR/`echo $TEST_NAME | sed s/arglist$/txt/`"

  #echo lang_parser_test $OPTIONS $INPUT
  if lang_parser_test $OPTIONS $INPUT > $OUTPUT 2> $LOG_FILE
  then
...
##
# Must put BUILD_SRC first in PATH so we get the correct version of chsmc.
##
PATH=$BUILD_SRC/c++/chsmc:$PATH

trap "x=$?; rm -f /tmp/*_$$_* 2>/dev/null; exit $x" EXIT HUP INT TERM

case $TEST in
*.arglist)  run_arglist_file ;;
esac
