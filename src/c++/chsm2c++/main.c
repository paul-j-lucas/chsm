...
//
char const	compiler::ident_[] = "CHSM-to-C++ Compiler";
char const	compiler::usage_[] = "[-EPSvY] file.chsmc file.h file.c";

extern void	emit_line_directive( std::ostream& );
...
//*****************************************************************************
{
	...
	//
	for ( int opt; (opt = ::getopt( argc, (char**)argv, "EPSvY" )) != EOF; )
		switch ( opt ) {
			...
			case 'P': // Don't generate #line directives.
				g.options_.no_line_ = true;
				break;
	...
	//
	// There should be 3 arguments unless -E is specified in which case
	// there should be 1.
	//
	argc -= ::optind, argv += ::optind;
	if ( argc != 1 + 2 * !g.options_.to_stdout_ )
		compiler::usage();
	...
	//
	EMIT	<< "#ifndef "<< g.user_code_->make_name() ENDL
		<< "#define "<< g.user_code_->make_name() ENDL
		ENDL
		<< compiler::ident_stamp ENDL
		ENDL
		<< "#include <chsm.h>" ENDL
		<< "namespace " << chsm_info::chsm_ns_alias_
		<< " = CHSM_NS;" ENDL
		ENDL
