...
char const transition_info::vector_suffix_[] = "_transitions";
...
// PARAMETERS
//
//      condition_id    The index ID of the condition to perform, if any.
//
...
//
//      action_id       The index ID of the action to perform, if any.
//
//*****************************************************************************
...
//*****************************************************************************
{
    ...
    if ( condition_id_ )
        o << '(' << chsm_info::chsm_ns_alias_
          << "::transition::condition)&" << g.chsm_->name() << "::"
          << chsm_info::condition_prefix_ << condition_id_;
    else
        o << 0;
    ...
    if ( target_id_ )
        o << '(' << chsm_info::chsm_ns_alias_
          << "::transition::target)&" << g.chsm_->name() << "::"
          << chsm_info::target_prefix_ << target_id_;
    else
        o << 0;
    ...
    if ( action_id_ )
        o << '(' << chsm_info::chsm_ns_alias_
          << "::transition::action)&" << g.chsm_->name() << "::"
          << chsm_info::action_prefix_ << action_id_;
    else
        o << 0;
