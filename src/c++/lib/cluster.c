...
**      src/c++/lib/cluster.c -- Run-Time library implementation
**
...
bool cluster::enter( event const &trigger, state *from_child ) {
    ...
    if ( from_child ) {
        ...
        // must merely make a note of that fact; it will take care of entering
        // itself.
        //
        active_child_ = last_child_ = from_child;
    } else {
        if ( !last_child_ || !history_ ) {
            //
            // We were never active before (and thus have no last_child_) or we
            // have no history: in either case, we must enter our default child
            // state.
            //
            last_child_ = &front();
        }
...
bool cluster::switch_child( state *child ) {
