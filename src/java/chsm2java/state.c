...
//*****************************************************************************
{
    o << indent << "public final " << class_name() << ' '
      << state_base( symbol_->name() ) << "; // " << ::serial( symbol_ );
    return o;
}
...
//*****************************************************************************
{
    ...
    char const *const chsm_ref
        = chsm_info::emitting_constructor_ ? "this" : "chsmM";

    o   << "( " << chsm_ref << ", \"" << symbol_->name() << "\", "
        << ( parent_ ?
            //
    ...
    if ( action_.enter_ )
        o << "new Thunk_"
          << chsm_info::enter_prefix_ << chsm_info::action_prefix_
          << m_name << "()";
    else
        o << "null";
    o << ", ";
    if ( action_.exit_ )
        o << "new Thunk_"
          << chsm_info::exit_prefix_ << chsm_info::action_prefix_
          << m_name << "()";
    else
        o << "null";
    o << ", ";
...
//*****************************************************************************
{
    o << state_base( symbol_->name() ) << " = new "
      << ( this == INFO_CONST(state, ROOT_SYM) ? "State_root" : class_name() );
